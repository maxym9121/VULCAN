...
type_list = ["U20"] + ["int" for i in range(num_ele)] + ["float"]
...
class InitialAbun(object):
    ...
    def ini_fc(self, data_var, data_atm):
        ...
        if vulcan_cfg.use_ion == True:
            ...
            copyfile(
                str(Path('.') / "fastchem_vulcan" / "input" / "parameters_wo_ion.dat"),
                str(Path('.') / "fastchem_vulcan" / "input" / "parameters.dat"),
            )
        ...
        with open(solar_ele, "r") as f:
            ...
            if vulcan_cfg.use_solar == True:
                ...
                for line in f.readlines():
                    ...
                    if sp in ele_list:
                        ...
                        sp in fc_list
                    ):  # other elements included in fastchem but not in VULCAN
                        sol_ratio = li[1].strip()
                        # print (sp + ":  " + str(sol_ratio))
                        if hasattr(
                            vulcan_cfg, "fastchem_met_scale"
            ...
            with open(str(Path('.') / "fastchem_vulcan" / "input" / "element_abundances_vulcan.dat"), "w") as f:
                f.write(new_str)

        # write a T-P text file for fast_chem
        with open(str(Path(".") / "fastchem_vulcan" / "input" / "vulcan_TP" / "vulcan_TP.dat"), "w") as f:
            ost = "#p (bar)    T (K)\n"
        ...
        base_path = str(Path(".") / "input" / "config.input")

        cwd_proc = str(Path(".") / "fastchem_vulcan")

        if sys.platform == "win32":
            executable = "fastchem.exe"
        else:
            executable = "./fastchem"

        command = f"{executable} {base_path}"

        try:
            subprocess.check_call(
                command, shell=True, cwd=cwd_proc
    ...
    def ini_y(self, data_var, data_atm):
        ...
        if vulcan_cfg.ini_mix == "EQ":
            ...
            fc = np.genfromtxt(
                str(Path('.') / 'fastchem_vulcan' / 'output' / 'vulcan_EQ.dat'),
            ...
            if sys.platform == "win32":
                command = "del vulcan_EQ.dat"
            else:
                command = "rm vulcan_EQ.dat"

            cwd_proc = str(Path(".") / "fastchem_vulcan" / "output")
            # remove the fc output
            subprocess.call(command, shell=True, cwd=cwd_proc)

        elif vulcan_cfg.ini_mix == "vulcan_ini":
            print(
                "Initializing with compositions from the prvious run "
                + vulcan_cfg.vul_ini
            )
            ...
            table = np.genfromtxt(
                vulcan_cfg.vul_ini, names=True, dtype=None, skip_header=1
            )
            ...
            print(
                "Initializing with constant (well-mixed): " + str(vulcan_cfg.const_mix)
            )
            for sp in vulcan_cfg.const_mix.keys():
                y_ini[:, species.index(sp)] = (
                    gas_tot * vulcan_cfg.const_mix[sp]
        ...
        if vulcan_cfg.use_condense:
            for sp in vulcan_cfg.condense_sp:
                ...
                if vulcan_cfg.use_ini_cold_trap:
                    ...
                    if (
                        vulcan_cfg.ini_mix != "table"
                        and vulcan_cfg.ini_mix != "vul_ini"
                    ):
                        ...
                        if list(
                            ...
                            conden_min_lev = np.where(data_atm.sat_mix[sp] == min_sat)[
                                0
                            ][0]
        ...
        if vulcan_cfg.use_condense:
            exc_conden = [
                _ for _ in range(ni) if species[_] not in vulcan_cfg.non_gas_sp
            ]
...
class Atm(object):
    ...
    def load_TPK(self, data_atm):
        ...
        # for atm_type = 'file' and also Kzz_prof = 'file
            ...
            if self.Kzz_prof == "file":
                atm_table = np.genfromtxt(
                    vulcan_cfg.atm_file, names=True, dtype=None, skip_header=1
                )
            ...
            if max(p_file) < data_atm.pco[0] or min(p_file) > data_atm.pco[-1]:
                print(
                    "Warning: P_b and P_t assgined in vulcan.cfg are out of range of the input.\nConstant extension is used."
                )
            ...
            if self.use_Kzz and self.Kzz_prof == "file":
                ...
                PTK_fun["pK"] = interpolate.interp1d(
                    ...
                    bounds_error=False,
                    fill_value=(
                        Kzz_file[np.argmin(p_file)],
                        Kzz_file[np.argmax(p_file)],
                    ),
                )
        ...
        if self.Kzz_prof == "const":
            ...
            data_atm.Kzz = (
                vulcan_cfg.K_max
                * (vulcan_cfg.K_p_lev * 1e6 / (data_atm.pico[1:-1])) ** 0.4
            )
    ...
    def f_mu_dz(
        self, data_var, data_atm, output
    ):  # Initilising mean molecular weight and dz
        ...
        if self.use_settling:
            ...
            dmu = (
                a * data_atm.Tco**na / (b + data_atm.Tco)
            )  # g cm-1 s-1 dynamic viscosity
            ...
            for sp in vulcan_cfg.non_gas_sp:
                try:
                    ...
                    print(sp + " has not been prescribed size and density!")
                    raise
    ...
    def read_sflux(self, var, atm):
        ...
        raw_right_indx = (
            np.searchsorted(atm.sflux_raw["lambda"], bins[-1], side="right") - 1
        )
        ...
        if not "sflux_din12_indx" in vars(var).keys():
            var.sflux_din12_indx = -1

        sum_bin = dbin1 * np.sum(var.sflux_top[: var.sflux_din12_indx])
        sum_bin -= (
            dbin1 * 0.5 * (var.sflux_top[0] + var.sflux_top[var.sflux_din12_indx - 1])
        )
        sum_bin += dbin2 * np.sum(var.sflux_top[var.sflux_din12_indx :])
        sum_bin -= (
            dbin2 * 0.5 * (var.sflux_top[var.sflux_din12_indx] + var.sflux_top[-1])
        )

        print(
            "The stellar flux is interpolated onto uniform grid of "
            + str(vulcan_cfg.dbin1)
            + " (<"
            + str(vulcan_cfg.dbin_12trans)
            + " nm) and "
            + str(vulcan_cfg.dbin2)
            + " (>="
            + str(vulcan_cfg.dbin_12trans)
            + " nm)"
            + " and conserving "
            ...
            with open(vulcan_cfg.bot_BC_flux_file) as f:
                for line in f.readlines():
                    if not line.startswith("#") and line.strip():
                        li = line.split()
                        # print(line)
                        atm.bot_fix_sp[species.index(li[0])] = li[2]
    ...
    def sp_sat(self, atm):
        ...
        for sp in vulcan_cfg.condense_sp:
            ...
            if sp == "H2O":
                ...
                saturate_p = (T < 0) * (
                    c0 * np.exp((c1 * T + T**2 / c2) / (T + c3))
                ) + (T > 0) * (w0 * np.exp((w1 * T + T**2 / w2) / (T + w3)))
                ...
                h2s_ice_log10 = (
                    -1329.0 / T + 9.28588 - 0.0051263 * T
                )  # 164.9 <= T <= 187.6
                h2s_l_log10 = -1145.0 / T + 7.94746 - 0.00322 * T  # 187.6 < T <= 213.2

                saturate_p = 10 ** (
                    (T <= 187.6) * h2s_ice_log10 + (T > 187.6) * h2s_l_log10
                )
...
if __name__ == "__main__":
    print(
        "This module stores classes for constructing atmospheric structure \
