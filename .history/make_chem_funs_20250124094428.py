...
def read_network():
    ...
    if vulcan_cfg.use_photo:
        ...
        ofstr = "# Chemical Network without Photochemistry \n\n"
    ...
    with open(vulcan_cfg.network, "r") as f:
        for line in f.readlines():
            ...
            if line.startswith("# 3-body"):
                ...
                special_re = (
                    False  # switch to reactions with special forms (hard coded)
                )
                # conden_re = True
                re_label = "#C"
...
def check_conserv():
    ...
    conserv_check = True
    compo = np.genfromtxt(vulcan_cfg.com_file, names=True, dtype=None, encoding="utf-8")
    compo_row = list(compo["species"])
    # Convert bytes to strings
    # print (compo_row)
    num_atoms = (
        len(compo.dtype.names) - 2
