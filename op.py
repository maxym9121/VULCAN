...
class ReadRate(object):
    ...
    def rev_rate(self, var, atm):
        ...
        print (f'Reverse rates from R1 to R{var.stop_rev_indx-2}')
    ...
    #                 # end if not
    #              # ========================================================================================
    #             elif special_re  and line.strip() and not line.startswith("#") and end_re == False:
    #
    #                 Rindx[i] = int(line.partition('[')[0].strip())
    #                 Rf[i] = line.partition('[')[-1].rpartition(']')[0].strip()
    #
    ...
    #
    #             # setting photo dissociation reactions to zeros
    #             elif photo_re  and line.strip() and not line.startswith("#") and end_re == False:
    #
    #                 #k[i] = np.zeros(nz)
    #                 Rf[i] = line.partition('[')[-1].rpartition(']')[0].strip()
    #
    ...
    #
    #             # end_re 
    #             elif br_read  and not line.startswith("#"):
    ...
    def make_bins_read_cross(self,var,atm):
        ...
        for n, sp in enumerate(absp_sp):   
            ...
            if sp in vulcan_cfg.T_cross_sp: 
                ...
                for tt in T_list:
                    path_tt = f'{vulcan_cfg.cross_folder}{sp}/{sp}_cross_{tt}K.csv'
                    if vulcan_cfg.use_ion: # usually the T-dependent cross sections are only measured in the photodissociation-relavent wavelengths so cross_tot = cross_diss
                        ...
                        cross_T_raw[(sp, tt)] = np.genfromtxt(path_tt,dtype=float,delimiter=',',skip_header=1, names = ['lambda','cross','disso','ion'])
                    ...
                    else: cross_T_raw[(sp, tt)] = np.genfromtxt(path_tt, dtype=float,delimiter=',',skip_header=1, names = ['lambda','cross','disso'])
        ...
        if vulcan_cfg.use_ion: 
            for sp in ion_sp:
                ...
                for i in range(1,var.ion_branch[sp]+1): # fill_value extends the first and last elements for branching ratios
                    ...
                    except: print(f"The ionic branches in the network file does not match the branchong ratio file for {sp}")
        ...
        for sp in vulcan_cfg.scat_sp:
            ...
            scat_raw[sp] = np.genfromtxt(f'{vulcan_cfg.cross_folder}rayleigh/{sp}_scat.txt',dtype=float,\
            skip_header=1, names = ['lambda','cross'])
...
class Integration(object):
    ...
    def __init__(self, odesolver, output):
        ...
        if vulcan_cfg.use_photo: self.update_photo_frq = vulcan_cfg.ini_update_photo_frq
        ...
        while not self.stop(var, para, atm): # Looping until the stop condition is satisfied
            ...
            if vulcan_cfg.use_photo and var.longdy < vulcan_cfg.yconv_min*10. and var.longdydt < 1.e-6:  
                ...
                if not para.switch_final_photo_frq:
                    ...
                    print (f'update_photo_frq changed to {vulcan_cfg.final_update_photo_frq}\n')
            ...
            if vulcan_cfg.use_condense and var.t >= vulcan_cfg.start_conden_time and not para.fix_species_start:
                ...
                if vulcan_cfg.fix_species and var.t > vulcan_cfg.stop_conden_time:
                    ...
                    if not para.fix_species_start: # switch to run for the first time
                        ...
                        print (f'rtol changed to {vulcan_cfg.rtol} after fixing the condensaed species.')
                        atm.vs *= 0
                            ...
                            if vulcan_cfg.fix_species_from_coldtrap_lev:
                                ...
                                if sp == 'H2O_l_s' or sp == 'H2SO4_l' or sp == 'NH3_l_s' or sp == 'S8_l_s': atm.conden_min_lev[sp] = nz-1 # fix condensates through the whole amtosphere 
                                    ...
                                    if list(var.y[conden_status,species.index(sp)]): # if it condenses
                                        ...
                                        print (f"{sp} is now fixed from {atm.pco[atm.conden_min_lev[sp]]*1e-6:.2f} bar.")
                                    else:
    ...
    def conv(self, var, para, atm, out=False, print_freq=100):
        ...
        # TEST
        if para.count %100==0: print (f"conv_indx: {indx}")
        ...
        longdy = np.max( longdy[ymix>0]/ymix[ymix>0] )
    ...
    def stop(self, var, para, atm):
        ...
        if var.t > vulcan_cfg.trun_min and para.count > vulcan_cfg.count_min and self.conv(var, para, atm):
            ...
            print (f'Integration successful with {para.count} steps and long dy, long dydt = {var.longdy} ,{var.longdydt}\nActinic flux change: {var.aflux_change:.2e}') 
            ...
            print (f'Integration not completed...\nMaximal allowed runtime exceeded ({vulcan_cfg.runtime} sec)!')
            para.end_case = 2
            return True
            ...
            print (f'After ------- {time.time()- para.start_time} seconds ------- s CPU time')
            print (f'Integration not completed...\nMaximal allowed steps exceeded ({vulcan_cfg.count_max})!')
            para.end_case = 3
            return True
...
#         atm.Hpi = Hpi
    ...
    def step_ok(self, var, para, loss_eps = vulcan_cfg.loss_eps, rtol = vulcan_cfg.rtol):
        if np.all(var.y>=0) and np.max( np.abs( np.fromiter(var.atom_loss.values(),float) - np.fromiter(var.atom_loss_prev.values(),float) ) )<loss_eps and para.delta<=rtol:
            return True
        else:
            return False
    ...
    def step_reject(self, var, para, loss_eps = vulcan_cfg.loss_eps, rtol = vulcan_cfg.rtol):
        ...
        else: # meaning np.max( np.abs( np.abs(y_loss) - np.abs(loss_prev) ) )<loss_eps
            para.loss_count +=1
            if vulcan_cfg.use_print_prog :
...
class Ros2(ODESolver):
    ...
    def solver(self, var, atm, para):
        ...
        # if para.count == 0 and vulcan_cfg.use_condense  and species.index("H2O") in self.fix_sp_bot_index: # only do once at count = 0
        ...
        if vulcan_cfg.use_condense  and para.fix_species_start :
            for sp in vulcan_cfg.fix_species:
                if not vulcan_cfg.fix_species_from_coldtrap_lev: # if Ptop is not specified, fix the whole column # TEST2022
                    pass
                else:
        ...
        if vulcan_cfg.use_botflux  or vulcan_cfg.use_fix_sp_bot: delta[0] = 0
    ...
    def naming_solver(self, para):
        ...
        # if vulcan_cfg.use_fix_all_bot :
        #     if vulcan_cfg.use_moldiff : print ('Use fixed bottom BC and molecular diffusion.')
        #     else: print ('Use fixed bottom BC and No molecular diffusion.')
        #     para.solver_str = 'solver_fix_all_bot'
            
        #else:
        if vulcan_cfg.use_moldiff : print ('Include molecular diffusion.')
...
class Output(object):
    ...
    def print_end_msg(self, var, para ): 
        ...
        for atom in vulcan_cfg.atom_list: print (f'{atom}: {var.atom_loss[atom]} ')
